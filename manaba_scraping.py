...
def login_wd(dt_now):
    # 現在の年度を取得
    fiscal_year = dt_now - relativedelta(months=4) + datetime.timedelta(hours=9)
...
def create_message(dt_now):
    ...
    for i in range(0, len(df)):
            ...
            if df.loc[i, 7] != "/icon-coursedeadline-off.png": # 課題
                if df.loc[i, 0] != "infoss情報倫理" and df.loc[i, 0] != "infoss情報倫理（2年生以上）": # infossは授業科目ではないが課題が表示され続けるため除外
